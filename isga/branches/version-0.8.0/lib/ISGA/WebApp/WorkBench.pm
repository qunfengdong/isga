...
sub WorkBench::Blast {
	...
	if ($form->ok) {
		...
		my $command = "___blast_executable___ -p $blast_program -i $runpath -o $blast_output -e $evalue -d \"$database\" -F $blastfilter -M $scoringmatrix";
...
sub WorkBench::Notify{
  ...
  $self->redirect( uri => "/WorkBench/Results/Blast?jobid=$job" );
}
