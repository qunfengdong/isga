...
=item public void HawkeyeInputUpload();

Uploads files for Hawkeye Workbench jobs.  This hopefully helps with 
handling large files

=cut
#------------------------------------------------------------------------
sub WorkBench::HawkeyeInputUpload{
  my $self = shift;
  my $web_args = $self->args;
  my $account = ISGA::Login->getAccount;
  my $upload_file = $self->apache_req->upload('upload_file');
  my $format = $web_args->{format};
  my %args = ( Type => 'Newbler Assembler Output' );

           $args{Upload} = $upload_file->fh;
           $args{Format} = $format;
           $args{Username} = basename($upload_file);
           my $file = ISGA::Job::Hawkeye->buildQueryFile(%args);
#           $file->stage($out_directory);

  $self->redirect( uri => '/UploadSuccess?file=$file' );
}
