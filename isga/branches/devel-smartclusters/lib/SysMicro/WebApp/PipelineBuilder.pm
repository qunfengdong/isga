...
sub PipelineBuilder::EditWorkflow {
  ...
  $self->redirect
    ( uri => "/PipelineBuilder/EditWorkflow?pipeline_builder=$pipeline_builder");
}
...
sub PipelineBuilder::ChooseComponent {
 ...
 if ( $form->ok ) {
    ...
    $self->redirect( uri => "/PipelineBuilder/EditWorkflow?pipeline_builder=$pipeline_builder" );
 }
