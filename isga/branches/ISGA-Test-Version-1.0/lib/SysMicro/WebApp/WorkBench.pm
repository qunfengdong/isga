...
sub WorkBench::Blast {
	...
	if ($form->ok) {
		...
		my %hash = (
								"pro.nr" => $database_path."nr/nr",
								"nuc.nr" => $database_path."nr/nt",
								"pro.uniref" => $database_path."asgard/UniRef100",
								"nuc.uniref" => '',
								"pro.allgroup" => $database_path."all_group/AllGroup.niaa",
								"nuc.allgroup" => $database_path."all_group/AllGroup.fasta",
							);
		...
		my $sge=SysMicro::SGEScheduler->new(
			...
			-output_file => '___tmp_file_directory___'."workbench/sge_log/".time.$$.".sgeout",
			-error_file => '___tmp_file_directory___'."workbench/sge_log/".time.$$.".sgeerror",
			);
...
sub WorkBench::MSA {
	...
	if ($form->ok){
		...
		my $time = time.$$;
		## Hard coded path
		#my $files_path = '/research/projects/sysmicro/workbench/msa/';
		my $files_path = '___tmp_file_directory___'."workkbench/msa/";
		my $pipeline_path = '___ergatis_output_repository___'.'cgb_format/';
		##
		...
		my $sge = SysMicro::SGEScheduler->new(
			-executable => {qsub=>'/cluster/sge/bin/sol-amd64/qsub -q seq', qstat=>'/cluster/sge/bin/sol-amd64/qstat'},
			-output_file => '___tmp_file_directory___'."workbench_sge/msa_$time.sgeout",
			-error_file => '___tmp_file_directory___'."workbench_sge/msa_$time.sgeerror"
		);
		...
		$jobid = SysMicro::Job->create(%sgejob_args) or X->throw(message => 'Error submitting job.');
		$jobid->addInputToCollection($input_file);
		$jobid->buildConfigFile( input => $input_file->getId, web_args => $web_args, input_file => $time );
...
sub WorkBench::ProtPars {
  ...
  my $outfile_present = $infile."_protpars_output";
	...
	if ($form->ok){
		...
		$self->redirect( uri => "/WorkBench/Results/ProtPars?jobid=$jobid&infile=$infile");
	}
