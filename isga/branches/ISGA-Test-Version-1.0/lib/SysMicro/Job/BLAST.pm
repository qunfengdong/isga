package SysMicro::Job::BLAST;
#------------------------------------------------------------------------

=head1 NAME

<SysMicro::Job::BLAST> provides methods for working with BLAST jobs.
...
sub buildConfigFile{
  ...
  foreach (keys %{$args{web_args}}) {
    unless ($_ eq 'query_sequence' || $_ eq 'upload_file' || $_ eq 'sequence_database' || $_ eq 'workbench_blast'){
      push(@params, {$_ => ${$args{web_args}}{$_}});
    }
  }
  my $config = {};
  $config->{job_id} = $self->getId;
	$contig->{input_file} = $args{input};
