package ISGA::Job::SffToFasta;
#------------------------------------------------------------------------

=head1 NAME

<ISGA::Job::SffToFasta> provides methods for working with FileConverison jobs.
...
#  my $collection = $self->getFileCollection();
  ...
  my $username = time."_sff2fasta_input";
...
sub buildConfigFile{
  ...
  my $tmp = '___tmp_file_directory___' . time . $$ . 'sff2fastaConfig.yaml';
...
sub addOutputToCollection {
  ...
  $output_file = ISGA::File->upload( $fh,
                                         ...
                                         Type => ISGA::FileType->new( Name => $type),
                                         Format => ISGA::FileFormat->new( Name => $format ),
