...
sub RunBuilder::UploadFile {
  ...
  if ( $form->ok ) {
    ...
    my %args = ( UserName => $form->get_input('file_name'),
		 Type => $pipeline_input->getType,
		 Format => $pipeline_input->getFormat,
		 Description => $form->get_input('description'),
    ...
    #### TODO - add code to handle file collections
...
sub RunBuilder::SelectInputList {
  ...
  # delete old_file if it was a pipeline input collection
