...
sub Blast {
  ...
  my @database_names = ( 'NCBI: nt', 'NCBI: nr' );
  my @database_values = ( 'nt', 'nr' );

my $component = SysMicro::Component->new(ErgatisName => 'ncbi-blastp.asgard');
my $component_builder = SysMicro::ComponentBuilder->new($component);
my $form_params = [ $component_builder->getForm() ];

use Data::Dumper;
  ${${$form_params}[0]}{TITLE} = "Change Title Here";
  push(@{${${${${$form_params}[0]}{sub}}[0]}{sub}}, {templ => 'text',
                         NAME => 'just_a_test',
                         TITLE => 'Just a Test',
                         VALUE => 'KASHI'});
warn Dumper($form_params);
	...
	my $content = '';
	if ($id){
		my $feature = SysMicro::GenomeFeature->new(Id => $id);
		#$content = $feature->getUniqueName;
		$content = $feature->getUpstream($upstream);
		$content .= $feature->getResidues;
		$content .= $feature->getDownstream($downstream);
	}
	...
	(
		...
		sub =>
			[
				...
				HINT => '<a href="/blast_sample.fsa">Sample file</a>',
				VALUE => $content,
				},
				{
				templ => 'upload',
