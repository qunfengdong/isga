...
use SysMicro::ComponentTemplate;
...
=item public SysMicro::ComponentBuilder new(Component $component);

Initialize the Componentbuilder object corresponding to the supplied 
Component.

=item public SysMicro::ComponentBuilder new(Component $component, ParameterMask $mask);
...
#------------------------------------------------------------------------
  sub _initializeComponent {
    ...
    if ( exists $clone->{RunBuilderParams} ) {
      ...
      $clone->{RunBuilder}{FormEngine}{TITLE} = $component->getName;
...
=item public SysMicro::ComponentBuilder injectMaskValues(ParameterMask $mask, Component $component);
...
#------------------------------------------------------------------------
  ...
  my $schema = YAML::LoadFile('___include_root___/schema/component_definition_kwalify.yaml');

  foreach my $template ( @{SysMicro::ComponentTemplate->query()} ) {
    
    my $file = '___include_root___/component_definition/' . $template->getFormPath;
    ...
    if ( -f $file ) {
      ...
      foreach ( @{SysMicro::Component->query( Template => $template )} ){
