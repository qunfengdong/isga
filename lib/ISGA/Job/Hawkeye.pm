package ISGA::Job::Hawkeye;
#------------------------------------------------------------------------

=head1 NAME

<ISGA::Job::Hawkeye> provides methods for working with Hawkeye jobs.
...
#  my $collection = $self->getFileCollection();
  ...
  my $username = time."_hawkeye_input";
  ...
  if ( $fh ) {
    ...
    $input_file = ISGA::File->upload($fh,
					 ...
					 Description => 'Input file for conversion for Hawkeye',
...
This method creates the ConfigFile for a Hawkeye job in yaml format.
The file contains information about the job id, the parameters, and 
the input file.
...
sub buildConfigFile{
  ...
  my $tmp = '___tmp_file_directory___' . time . $$ . 'hawkeyeConfig.yaml';
  ...
  my %args = ( Type => ISGA::FileType->new( Name => 'Toolbox Job Configuration'),
               ...
               CreatedAt => ISGA::Timestamp->new(),
               CreatedBy => ISGA::Login->getAccount,
               ExistsOutsideCollection => 0, Description => 'Config for assembler output conversion for Hawkeye' );
...
sub addOutputToCollection {
  ...
  $output_file = ISGA::File->upload( $fh,
                                         ...
                                         Type => ISGA::FileType->new( Name => 'Hawkeye Input'),
                                         Format => ISGA::FileFormat->new( Name => 'Amos Bank' ),
                                         Description => 'Hawkeye Input produced by workbench job',
                                         CreatedBy => ISGA::Login->getAccount,
                                         CreatedAt => ISGA::Timestamp->new(),
                                         IsHidden => 0,
                                         ExistsOutsideCollection => 0
