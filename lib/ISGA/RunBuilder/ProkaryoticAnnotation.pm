...
=item public void verifyUpload( filehandle $fh, string $name, PipelineInput $pi);

Performs any needed verification for a file uploaded to a pipeline.

=cut 
#------------------------------------------------------------------------
sub verifyUpload {

  my ($self, $fh, $name, $pi) = @_;

  my $ci = $pi->getClusterInput->getName;

   # we know how to verify fasta file inputs
  if ( $ci eq 'Genome_Contigs' ) {
    my $data = ISGA::FileFormat::FASTA->verify( $fh, UserName => $name, Alphabet => 'nucleotide' );
  }
}
