...
sub Blast {
  ...
  my @database_values = ( 'nt', 'nr', 'tair9_cdna', 'tair9_pep' );
	...
	foreach my $run(@$runs){
	    ...
	    next unless $run->getGlobalPipeline == $prok_annotation_pipeline;
...
=item public hashref form SffToFasta();

Build form for converting SFF files to FASTA
...
sub SffToFasta {
  ...
  $form->conf( { ACTION => '/submit/WorkBench/SffToFasta',
                 FORMNAME => 'workbench_sfftofasta',
                 FORMATTR => [ 'onsubmit="return startEmbeddedProgressBar(this)"' ],
                 SUBMIT => 'Convert SFF to FASTA',
                 ENCTYPE => 'multipart/form-data',
                 sub => \@form } );
