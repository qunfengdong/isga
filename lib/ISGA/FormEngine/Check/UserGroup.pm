package ISGA::FormEngine::Check::UserGroup;
#------------------------------------------------------------------------
...
sub nameIsAvailable {
  ...
  # check values
  return  ( ISGA::Party->exists(Name => $name) ? $error : '' );
}
...
#------------------------------------------------------------------------
sub isMine {
  shift->getOwnedBy == ISGA::Login->getAccount ? '' : 'You do not own this group.';
}
...
ISGA::FormEngine::SkinUniform->_register_check('UserGroup::nameIsAvailable');
ISGA::FormEngine::SkinUniform->_register_check('UserGroup::isMine');
ISGA::FormEngine::SkinUniform->_register_check('UserGroup::ownsInvitation');
